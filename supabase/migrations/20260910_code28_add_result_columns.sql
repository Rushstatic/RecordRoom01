...
ALTER TABLE IF EXISTS public.malaria_blood_samples ADD COLUMN IF NOT EXISTS result text DEFAULT 'Pending';
...
-- Add result tracking columns to tb_records (as requested, though codebase uses tb_suspected_patient_register)
ALTER TABLE IF EXISTS public.tb_records ADD COLUMN IF NOT EXISTS result text DEFAULT 'Pending';
...
ALTER TABLE IF EXISTS public.tb_suspected_patient_register ADD COLUMN IF NOT EXISTS result text DEFAULT 'Pending';
