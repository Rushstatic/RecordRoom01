...
import { MainNavTabId, DATA_ENTRY_SUB_ITEMS, REPORTS_SUB_ITEMS, ADMIN_SUB_ITEMS } from '../types/navigation';
...
export const SubNavBar: React.FC<SubNavBarProps> = ({
  ...
  // If subcentre employee somehow has admin tab, fallback to data-entry
  if (currentTab === 'admin' && !isPhcController) {
    return null;
  }
  ...
  // For Subcentre Employee, simplify Data Entry: show only the active register cards on the screen
  if (currentTab === 'data-entry' && !isPhcController) {
    return null;
  }
