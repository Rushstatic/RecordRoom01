...
function processMalariaTargetsPage() {
  let content = fs.readFileSync('src/pages/MalariaTargetsPage.tsx', 'utf8');
  ...
  // Remove handlers body manually by regex
  ...
  content = content.replace(/if \(item\.subcentre_id && item\.subcentre_id !== selectedSubcentreId\) return false;/g, '');
  content = content.replace(/if \(selectedPhcId && item\.phc_id && item\.phc_id !== selectedPhcId\) return false;/g, '');
  content = content.replace(/if \(selectedSubcentreId && item\.subcentre_id && item\.subcentre_id !== selectedSubcentreId\) return false;/g, '');
  ...
  // Filter Logic in filteredEmployees
  const filteredEmployeesRegex = /const filteredEmployees = useMemo\(\(\) => \{[\s\S]*?\}, \[employees, subcentres, selectedEmployeeId, selectedSubcentreId, selectedPhcId, searchQuery\]\);/;
  const newFilteredEmployees = `const filteredEmployees = useMemo(() => {
    return availableEmployees.filter((e) => {
      let targetEmployeeId = selectedEmployeeId;
      if (isSubcentreStaff) targetEmployeeId = user?.employeeId || targetEmployeeId;
      if (targetEmployeeId && e.id !== targetEmployeeId) return false;
      if (searchQuery && !e.employee_name.toLowerCase().includes(searchQuery.toLowerCase())) return false;
      return true;
    });
  }, [availableEmployees, selectedEmployeeId, searchQuery, isSubcentreStaff, user]);`;
  content = content.replace(filteredEmployeesRegex, newFilteredEmployees);

  // Filter Logic in filteredSubcentres
  const filteredSubcentresRegex = /const filteredSubcentres = useMemo\(\(\) => \{[\s\S]*?\}, \[subcentres, selectedSubcentreId, selectedPhcId, isSubcentreStaff, searchQuery\]\);/;
  content = content.replace(filteredSubcentresRegex, `const filteredSubcentres = [];`);
  
  // UI - Remove PHC and Subcentre dropdowns
  const uiRegex = /<div className="flex items-center gap-2">[\s\S]*?<label className="text-xs font-bold text-slate-700 w-16 shrink-0">PHC<\/label>[\s\S]*?<select[\s\S]*?<\/select>[\s\S]*?<\/div>\s*<div className="flex items-center gap-2">[\s\S]*?<label className="text-xs font-bold text-slate-700 w-16 shrink-0">Subcentre<\/label>[\s\S]*?<select[\s\S]*?<\/select>[\s\S]*?<\/div>/;
  if (content.match(uiRegex)) {
    content = content.replace(uiRegex, '');
  }
  
  // Remove them if they are wrapped differently
  content = replaceBlock(
    content,
    /{?\/\* PHC & Subcentre \*\//,
    /<\/select>\s*<\/div>\s*<\/div>\s*\)\}/,
    ''
  );

  // We should also replace the whole Filters section properly because the layout might be broken
  content = replaceBlock(
    content,
    ...
    `{/* Dynamic Filters based on Tab */}
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            {activeTab === 'employees' && !isSubcentreStaff && (
              <div>
                <label className="block text-xs font-bold text-slate-700 mb-1">कर्मचारी</label>
                <select
                  value={selectedEmployeeId}
  ...
  fs.writeFileSync('src/pages/MalariaTargetsPage.tsx', content, 'utf8');
}

processMalariaTargetsPage();
